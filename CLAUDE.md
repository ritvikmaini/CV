...
- **About portrait:** `about.portrait` is a path under `public/` (e.g. `/portrait.jpg`) shown as a circular avatar in the About header (`AboutModal`); omit/empty to hide it. Drop the new image in `public/` first. The live site sits behind Cloudflare with `max-age=14400`, so **replacing an image in place is not enough** — a hard reload can still serve the old one for hours. Bump the version query on the path (`/portrait.jpg?v=3`) so the URL itself changes. The file itself is **pre-cropped square** with the face centred and headroom above it, so reframing means re-cropping the image (`sips -c S S --cropOffset Y X`), not nudging `objectPosition`. The blend (rim feather + bottom mask that fades the torso into negative space, the vignette, the photo filter) lives in `AboutModal.tsx` — `PORTRAIT_MASK` and the vignette gradient, not in content. `about.bio` and `about.languages` complete that section.
